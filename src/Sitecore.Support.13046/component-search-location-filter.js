    ...
    LocationFilterView = XA.component.search.baseView.extend({
        ...
        translateUserLocation: function(lookupQuery) {
            ...
            if (lookupQuery.text === "") {
				var data = {};
				data.coordinates = [];
				var mapElements = $(".map.component")[0];
				var mapCenter = JSON.parse(mapElements.dataset.properties);
				var mapLongitude = mapCenter.longitude;
				var mapLatitude = mapCenter.latitude;
				if(mapLatitude!= null && mapLongitude.length !== 0)
				{
		data.coordinates[0] = mapLatitude;
				}
				else
				{
		...
		data.coordinates[0] = 0;
				...
				if(mapLongitude!= null && mapLongitude.length !== 0)
				{		
			data.coordinates[1] =mapLongitude;
				}
				else
				{
			data.coordinates[1] = 0;
				...
				mapsConnector.clearMarkers($(mapElements).find(".map-canvas").prop("id"));
				mapsConnector.updateMyPoiLocation($(mapElements).find(".map-canvas").prop("id"),data.coordinates);
				hashObj = that.createHashObject(data.coordinates[0] + "|" + data.coordinates[1], properties.f + ",Ascending");
